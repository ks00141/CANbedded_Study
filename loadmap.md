...
이 로드맵의 목표는 `src/CBD`에 포함된 Vector CANbedded 계열 미들웨어 묶음을 학습하여, 동일한 구조의 C 언어 기반 미들웨어를 단계적으로 재구현할 수 있는 이해 순서와 구현 순서를 잡는 것이다. 재구현 학습의 타깃 MCU는 STMicroelectronics `SPC584C70` MCU로 설정한다.

`SPC584C70`은 ST의 SPC58 C Line 안에서 `SPC584Cx/SPC58ECx` 계열에 속하는 차량용 32-bit Power Architecture MCU이다. `SPC58EC70`과 같은 데이터시트/레퍼런스 매뉴얼 계열을 공유하고 CAN-FD 주변장치 구조도 매우 유사하지만, MCU 타깃으로는 구분해야 한다. `SPC584C70`은 single computing e200z420n3 코어 계열이고, `SPC58EC70`은 dual computing e200z420n3 코어 계열이다. 또한 C70 등급은 2 MB code flash 등급이며, RAM option 표 기준 `SPC584C70`은 `SPC58EC70`보다 RAM 구성이 작다. 따라서 startup, linker script, core-local RAM, interrupt target 설정을 두 MCU 간에 그대로 공유한다고 가정하면 안 된다.

공식 ST 제품 정보와 RM0407 기준으로, `SPC584Cx/SPC58ECx` 계열은 VLE 명령 집합, HSM 내부 e200z0 코어, shared Message RAM 구조의 8개 M_CAN 인터페이스와 ISO CAN-FD 지원을 제공한다. AN5416 기준으로 M_CAN은 두 subsystem으로 나뉘며 subsystem 안에서 Message RAM, clock, ECC 자원을 공유하므로 offset overlap을 피해야 한다. 정확한 동작 주파수, 온도 등급, 패키지, Flash option은 `SPC584C70E3GMC1X` 같은 full ordering part number와 DS11620 ordering information을 기준으로 확정한다. 따라서 학습용 미들웨어는 PC mock 환경에서 먼저 구현하되, 최종 구조는 `SPC584C70`의 M_CAN 기반 CAN-FD 컨트롤러, interrupt vector, Power Architecture endian/정렬 이슈, 메모리 배치, peripheral clock 설정을 고려해 설계한다.
...
| 4 | `middleware/can/can.*`, SPC584C70 CAN HAL adapter | HS-CAN 송수신, Rx dispatch, Tx confirmation |
...
```text
app -> uds -> isotp(classic) -> can(classic) -> SPC584C70 CAN controller
app -> il  -> can(classic)  -> SPC584C70 CAN controller
...
```text
app -> uds -> isotp(classic/fd) -> can(classic/fd) -> SPC584C70 CAN-FD controller
app -> il  -> can(classic/fd)   -> SPC584C70 CAN-FD controller
...
## 2.1. 타깃 MCU: SPC584C70 적용 관점

`SPC584C70` MCU를 대상으로 재구현할 때는 다음 관점을 처음부터 염두에 둔다.

- CPU/컴파일러 관점: single computing e200z420n3 기반 Power Architecture automotive MCU이므로 정렬, endian, interrupt ABI, VLE 컴파일 옵션, compiler intrinsic 사용 가능성을 고려한다. `SPC58EC70`용 dual-core startup/linker 설정을 그대로 가져오지 않는다.
- CAN 주변장치 관점: SPC584C70은 Bosch M_CAN 기반 ISO CAN-FD 인터페이스를 8개 제공하므로, CAN Driver는 channel instance, shared Message RAM offset, nominal/data bit timing, filter table, interrupt line을 설정으로 분리한다.
- 메모리 관점: Vector 코드의 `V_MEMROM`, `V_MEMRAM` 같은 메모리 클래스 매크로는 SPC584C70의 flash/RAM section 배치로 연결될 수 있다.
...
- ST `SPC584C70E3` product page: `SPC584Cx/SPC58ECx` 공통 기능과 8 M_CAN ISO CAN-FD feature 확인
- ST `DS11620` datasheet: C70 flash/RAM option, `SPC584Cx`와 `SPC58ECx` ordering 차이 확인
...
- 산출물: HS-CAN mock driver와 SPC584C70 HAL adapter 인터페이스
...
- BusOff interrupt 연결
- 타이머 tick 연결
- 산출물: SPC584C70 HS-CAN bring-up package
...
- 산출물: SPC584C70 CAN-FD 미들웨어 variant
...
- SPC584C70 HAL adapter는 mock HAL과 같은 상위 API를 제공해야 한다.
