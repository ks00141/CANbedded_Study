...
1. Q: CAN 메시지를 이름 대신 handle로 접근하는 이유는 무엇인가?
   A: 상위 계층이 메시지 이름 문자열이나 ID 값을 직접 알지 않아도 되게 하기 위해서이다. handle은 생성 설정과 Driver 사이의 안정적인 index 역할을 하며, variant별 ID 변경에도 API를 유지할 수 있다.

2. Q: `id`, `dlc`, `length`는 어떻게 구분해야 하는가?
   A: `id`는 arbitration field의 CAN identifier이고, `dlc`는 CAN frame header의 Data Length Code이며, `length`는 실제 payload byte 수이다. CAN-FD에서는 DLC와 length가 항상 같지 않으므로 둘을 분리한다.

3. Q: Classical CAN만 구현할 때도 CAN-FD 필드를 넣어야 하는가?
   A: 학습 프로젝트의 최종 목표가 CAN-FD까지 포함하므로 `is_fd`, `brs`, `length` 필드를 초기에 넣는 편이 좋다. Classical CAN에서는 `is_fd=0`, `brs=0`, `length<=8` 규칙으로 제한한다.

4. Q: CPU endian과 CAN signal endian은 같은 개념인가?
   A: 아니다. `SPC584C70`의 CPU endian 특성과 DBC signal byte order는 별개이다. signal packing/unpacking은 DBC가 정의한 Intel/Motorola byte order를 기준으로 구현해야 한다.

5. Q: Rx filter table과 Rx object table은 같은 것인가?
   A: 다르다. filter table은 M_CAN controller가 어떤 frame을 받을지 결정하고, Rx object table은 받은 frame을 상위 계층 handle로 매핑한다. 둘의 ID 정책이 맞지 않으면 interrupt는 와도 IL callback이 호출되지 않을 수 있다.

6. Q: extended ID와 standard ID를 같은 테이블에 넣어도 되는가?
   A: 가능하지만 `is_extended` 또는 IDE flag가 반드시 필요하다. 같은 numeric ID라도 standard와 extended frame은 서로 다른 frame format이므로 filter와 dispatch에서 구분해야 한다.

7. Q: payload buffer를 구조체 내부 배열로 둘지 pointer로 둘지 어떻게 결정하는가?
   A: 고정 message 수와 고정 payload라면 배열이 단순하다. CAN-FD와 large buffer, generated table, memory section 배치를 고려하면 pointer + length/capacity 구조가 유연하다.

8. Q: Tx와 Rx 데이터 모델을 하나로 합치면 안 되는가?
   A: 공통 `CanFrame`은 공유할 수 있지만 Tx 설정과 Rx 설정은 책임이 다르다. Tx는 transmit handle, mailbox, confirmation이 중요하고 Rx는 filter, dispatch handle, indication callback이 중요하다.

9. Q: 이 단계에서 실제 bus 송수신까지 확인해야 하는가?
   A: 본격 송수신은 4단계 Driver에서 확인한다. 3단계에서는 설정 테이블에서 `CanFrame`을 만들고, ID/DLC/length/data pointer가 의도대로 조립되는지 디버거로 확인한다.

10. Q: 데이터 모델 오류는 어떤 형태로 드러나는가?
    A: analyzer에는 정상 frame이 보이지만 상위 계층이 다른 message로 해석하거나, CAN-FD payload 일부가 잘리거나, DLC는 15인데 length를 15바이트로 오해하는 식으로 나타난다.

11. Q: 다음 IL 단계와 연결하려면 무엇을 준비해야 하는가?
    A: IL이 사용할 Tx/Rx handle, signal buffer pointer, update flag 위치, payload length, endian policy를 명확히 해야 한다. IL은 이 모델을 기준으로 signal packing/unpacking을 수행한다.
