...
타깃 MCU를 `SPC58EC70`으로 잡으면 공통 계층은 더 중요해진다. `SPC58EC70`은 dual e200z420n3 기반 Power Architecture 계열 차량용 MCU이며 VLE 명령 집합을 지원하므로 다음을 고려해야 한다.
...
SPC58EC70은 Power Architecture 계열 MCU이며 컴파일러, 링커 스크립트, 메모리 섹션 정책의 영향을 크게 받는다. 공통 헤더를 작성할 때는 PC 테스트용 정의와 실제 MCU 빌드용 정의를 분리하고, 같은 API 이름을 유지해야 이후 계층을 쉽게 이식할 수 있다.
