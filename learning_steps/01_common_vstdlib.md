...
SPC584C70은 Power Architecture 계열 MCU이며 컴파일러, 링커 스크립트, 메모리 섹션 정책의 영향을 크게 받는다. 공통 헤더를 작성할 때는 PC 테스트용 정의와 실제 MCU 빌드용 정의를 분리하고, 같은 API 이름을 유지해야 이후 계층을 쉽게 이식할 수 있다.
