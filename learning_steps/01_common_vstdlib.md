...
1. Q: `vuint8`, `vuint16` 같은 타입을 굳이 새로 정의하는 이유는 무엇인가?
   A: 미들웨어 전체가 compiler와 MCU에 독립적인 이름으로 타입을 공유하기 위해서이다. `SPC584C70` 포팅에서는 실제 폭이 8/16/32비트인지 compile-time assert와 디버거 watch로 확인해야 한다.

2. Q: `stdint.h`만 사용하면 충분하지 않은가?
   A: 내부 구현에는 `stdint.h`를 기반으로 삼는 것이 좋지만, 상위 CBD 스타일 API는 `vuint8` 같은 이름을 기대한다. 표준 타입과 미들웨어 타입 사이의 매핑을 이 단계에서 고정한다.

3. Q: `V_MEMROM`, `V_MEMRAM` 매크로를 비워 두어도 되는가?
   A: 초기 빌드에서는 비워 둘 수 있지만, 학습 완료 기준은 실제 linker section 정책과 연결 가능한 상태이다. 상수 테이블이 flash에, runtime buffer가 RAM에 배치되는지 map 파일로 확인한다.

4. Q: `const`만 붙이면 ROM 배치가 보장되는가?
   A: 항상 그렇지는 않다. 컴파일러 옵션, linker script, memory class macro에 따라 달라질 수 있다. `const`와 section attribute, linker map을 함께 검증해야 한다.

5. Q: `VStdRamMemCpy`와 `VStdRomMemCpy`를 나누는 이유는 무엇인가?
   A: 일부 embedded compiler는 ROM pointer와 RAM pointer의 address space qualifier가 다를 수 있다. 학습용 구현에서는 같은 함수로 처리되더라도 API 이름은 분리해 두면 이식성이 좋아진다.

6. Q: CAN-FD를 고려해 Common 계층에서 바꿔야 할 것은 무엇인가?
   A: 메모리 복사/초기화 함수가 8바이트 전용 가정을 하지 않아야 한다. CAN-FD payload 64바이트, ISO-TP 장문 응답, UDS buffer를 처리할 수 있도록 length와 buffer capacity를 명확히 다룬다.

7. Q: interrupt disable/restore wrapper는 언제 필요한가?
   A: ISR과 main function이 공유하는 Tx/Rx state, signal buffer, queue head/tail을 보호할 때 필요하다. 단순 disable 함수보다 이전 mask 상태를 저장하고 복구하는 형태가 안전하다.

8. Q: 모든 공유 변수에 `volatile`을 붙이면 안전한가?
   A: 아니다. `volatile`은 최적화로 인한 누락을 막을 뿐 atomicity나 race condition을 해결하지 않는다. 접근 단위와 critical section 정책을 함께 설계해야 한다.

9. Q: endian 문제는 Common 계층에서 해결해야 하는가?
   A: 기본 타입의 byte order 특성은 이해해야 하지만, CAN signal byte order는 데이터 모델 계층에서 처리한다. Common 계층은 CPU endian을 숨기기보다 상위 계층이 명확히 다루도록 타입과 유틸리티를 제공한다.

10. Q: 이 단계의 bring-up에서 무엇을 디버거로 봐야 하는가?
    A: 타입 크기, ROM/RAM symbol address, buffer 초기화 결과, 8/64바이트 복사 결과, interrupt wrapper 호출 전후 상태를 확인한다. 이 값이 이후 CAN buffer와 설정 테이블 검증의 기준이 된다.

11. Q: Common 계층 오류는 이후 단계에서 어떤 증상으로 나타나는가?
    A: 메시지 payload 깨짐, callback flag 미갱신, section overflow, UDS 응답 buffer 손상, CAN-FD payload 복사 실패로 나타난다. 따라서 첫 단계에서 작아 보이는 타입/메모리 오류를 가볍게 넘기면 안 된다.
