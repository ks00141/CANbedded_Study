...
1. Q: BusOff는 어떤 상태인가?
   A: CAN controller의 transmit error counter가 한계를 넘어 bus 참여가 중단된 상태이다. 이 상태에서는 정상 송신을 계속 시도하지 말고 통신 offline 전환과 복구 절차를 수행해야 한다.

2. Q: BusOff 감지는 Driver와 NM 중 어디의 책임인가?
   A: Driver는 M_CAN interrupt/register로 BusOff를 감지하고 알린다. NM 또는 BusOff manager는 복구 타이밍, 재초기화, online 복귀 조건을 정책으로 관리한다.

3. Q: BusOff가 발생하면 Tx queue는 어떻게 처리해야 하는가?
   A: 정책을 정해야 한다. 안전한 초기 구현은 송신을 차단하고 pending queue를 폐기하거나 명시적으로 failed 처리하는 방식이다. 무작정 보존하면 복구 직후 오래된 frame이 나갈 수 있다.

4. Q: 자동 복구와 수동 복구 중 무엇이 좋은가?
   A: 학습용으로는 상태를 관찰하기 쉬운 수동 또는 단계적 복구가 좋다. 제품 수준에서는 OEM 요구사항, network management 정책, safety 요구사항에 맞춰 자동 복구 횟수와 지연 시간을 정한다.

5. Q: M_CAN에서 BusOff 확인 시 어떤 register를 봐야 하는가?
   A: `IR.BO`, `PSR.BO`, `ECR.TEC/REC`, last error code를 함께 본다. interrupt flag만 보고 판단하면 clear 순서나 상태 전환을 놓칠 수 있다.

6. Q: BusOff 복구 중 CCL은 어떤 상태여야 하는가?
   A: 일반적으로 communication disabled 또는 offline 상태로 보고 Tx 요청을 막아야 한다. 복구 완료 후 Driver와 NM 상태가 정상일 때 CCL이 online으로 풀어 준다.

7. Q: BusOff는 어떻게 실습에서 유도할 수 있는가?
   A: bitrate mismatch, termination 제거, transceiver 연결 문제 등으로 유도할 수 있지만 보드와 네트워크를 보호해야 한다. 실습 전 analyzer와 bus topology를 확인하고 복구 가능한 조건에서만 수행한다.

8. Q: BusOff가 반복되면 소프트웨어만 고치면 되는가?
   A: 아니다. wiring, termination, transceiver enable, bitrate, CAN-FD data phase, sample point, ground reference 같은 물리/설정 문제를 함께 확인해야 한다.

9. Q: NM과 AUTOSAR NM은 같은 의미인가?
   A: 이 학습 단계의 NM은 BusOff와 통신 상태 복구를 다루는 단순 network management 성격이다. 실제 AUTOSAR NM 전체 기능과 동일하다고 보면 안 된다.

10. Q: 복구 완료 판단은 무엇으로 해야 하는가?
    A: controller가 init 또는 restricted state에서 정상 mode로 돌아오고, error counter와 BusOff flag가 안정화되며, analyzer에서 정상 frame 송수신이 확인되어야 한다. 소프트웨어 state만 online이면 부족하다.

11. Q: CAN-FD에서 BusOff 복구는 달라지는가?
    A: 절차 자체는 유사하지만 data phase timing, BRS, transceiver delay 문제가 추가된다. FD frame에서만 BusOff가 반복되면 `DBTP`, TDC, analyzer FD 설정을 우선 확인한다.
