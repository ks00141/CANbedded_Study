...
루트 보조 파일 계층은 특정 MCU/채널 매핑 보정처럼 여러 모듈에서 참조 가능한 보조 정의를 제공한다. 현재 파일명은 `RemapChannelSpc58.h`이지만, 내용상 핵심은 M_CAN 채널 번호와 Message RAM 관련 크기 매크로이다.
...
- 파일 종류: `.h`
- 파일 크기: 4,886 bytes
- 파일 내용: `RemapChannelSpc58.h` 파일명 기반의 SPC58 계열 보조 헤더이며, 실제 내용은 `MCAN1`, `MCAN6` 같은 M_CAN 채널 리맵과 Message RAM 크기 설정을 제공한다.
